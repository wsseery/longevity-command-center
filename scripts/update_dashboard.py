...
# ---- STEP 1: NEWS -> news_data.json (NON-FATAL: never blocks the Step 2 price refresh) ----
# A missing ANTHROPIC_API_KEY or any news error must not abort the run; prices still update.
api_key = os.environ.get("ANTHROPIC_API_KEY")
if not api_key:
    print("WARNING: ANTHROPIC_API_KEY not set; skipping news refresh (prices will still update). "
          "Add the secret in repo Settings -> Secrets and variables -> Actions to re-enable automated news.",
          file=sys.stderr)
else:
    try:
